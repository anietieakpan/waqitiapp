-- V401__Add_Standardized_DLQ_Records_Table.sql
-- Standardized DLQ infrastructure for account-service
...
-- The existing dlq_retry_queue and dlq_manual_review_queue tables (V400) continue
-- to exist for backward compatibility with existing DLQ infrastructure.
...
CREATE TABLE IF NOT EXISTS dlq_records (
    ...
    service_name            VARCHAR(100) NOT NULL DEFAULT 'account-service',
    ...
    -- Constraints
    CONSTRAINT chk_dlq_rec_status CHECK (status IN ('PENDING', 'REPROCESSED', 'PARKED', 'FAILED')),
    CONSTRAINT chk_dlq_rec_retry_count CHECK (retry_count >= 0),
    CONSTRAINT chk_dlq_rec_max_retries CHECK (max_retries > 0),
    CONSTRAINT unique_dlq_rec_kafka_message UNIQUE (topic, partition, offset)
);
...
CREATE INDEX idx_dlq_rec_pending_retry ON dlq_records(status, next_retry_time)
    WHERE status = 'PENDING';

-- Index for monitoring queries by topic
CREATE INDEX idx_dlq_rec_topic_status ON dlq_records(topic, status, created_at DESC);

-- Index for finding stuck messages
CREATE INDEX idx_dlq_rec_stuck_messages ON dlq_records(status, created_at)
    WHERE status = 'PENDING' AND retry_count >= 5;

-- Index for service-level queries
CREATE INDEX idx_dlq_rec_service_status ON dlq_records(service_name, status, created_at DESC);

-- Index for message ID lookups (unique index already exists, this is for coverage)
CREATE INDEX idx_dlq_rec_message_id ON dlq_records(message_id);

-- Index for reprocessing analysis
CREATE INDEX idx_dlq_rec_reprocessed ON dlq_records(reprocessed_at DESC)
    WHERE status = 'REPROCESSED';

-- Index for parked messages
CREATE INDEX idx_dlq_rec_parked ON dlq_records(parked_at DESC)
    WHERE status = 'PARKED';
...
CREATE OR REPLACE FUNCTION update_dlq_rec_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trg_dlq_rec_updated_at
    BEFORE UPDATE ON dlq_records
    FOR EACH ROW
    EXECUTE FUNCTION update_dlq_rec_updated_at();

-- ============================================================================
-- Comments (Documentation)
-- ============================================================================

COMMENT ON TABLE dlq_records IS 'Standardized DLQ records table for failed Kafka messages. Automatically processed by DLQProcessorService every 2 minutes. Complements existing dlq_retry_queue and dlq_manual_review_queue tables from V400.';
...
COMMENT ON COLUMN dlq_records.service_name IS 'Service that owns this DLQ record (account-service)';
...
-- ============================================================================
--
-- Existing DLQ infrastructure (V400) continues to be used by:
-- - BaseDlqHandler and all *DlqHandler classes
-- - DlqRetryProcessor
-- - dlq_retry_queue table
-- - dlq_manual_review_queue table
--
...
-- - Enables cross-service DLQ monitoring and recovery
--
-- Future migration can consolidate dlq_retry_queue and dlq_records,
-- deprecating the old structure once all handlers are migrated.
--
-- ============================================================================
...
--   * CompensatingTransactionStrategy: Not used in account-service
--   * ManualInterventionStrategy: Max retries exceeded, critical failures
--
-- Critical Account Topics:
-- - account-created: New account creation events
-- - account-activated: Account activation events
-- - account-closed: Account closure events
-- - account-suspended: Account suspension events
-- - account-frozen: Account freeze events
-- - kyc-verification: KYC verification completion
--
-- Monitoring:
