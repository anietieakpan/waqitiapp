...
@Service
@Slf4j
public class AccountSuspensionEventsConsumerDlqHandler extends BaseDlqHandler<Object> {
    private static final String HANDLER_NAME = "AccountSuspensionEventsConsumer";
    ...
    @KafkaListener(topics = "${kafka.topics.account-suspension-events.dlq:account.suspension.events.dlq}",
    ...
    protected RecoveryDecision classifyFailure(Object event, String exceptionMessage, String exceptionClass,
        ...
        if (containsAny(lowerMessage, "compliance", "kyc", "aml")) return RecoveryDecision.forComplianceBlock(exceptionMessage, retryAttempt);
        if (containsAny(lowerMessage, "invalid state", "cannot suspend")) {
            return RecoveryDecision.permanentFailure(PermanentFailureRecord.FailureCategory.INVALID_STATE,
                PermanentFailureRecord.BusinessImpact.HIGH, "Cannot suspend account: " + exceptionMessage);
        }
