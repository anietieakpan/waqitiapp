...
@Service
@Slf4j
public class CustomerOffboardingEventsConsumerDlqHandler extends BaseDlqHandler<Object> {
    private static final String HANDLER_NAME = "CustomerOffboardingEventsConsumer";
    ...
    @KafkaListener(topics = "${kafka.topics.customer-offboarding-events.dlq:customer.offboarding.events.dlq}",
    ...
    protected RecoveryDecision classifyFailure(Object event, String exceptionMessage, String exceptionClass,
        ...
        if (containsAny(lowerMessage, "active accounts", "open accounts")) {
            return RecoveryDecision.permanentFailure(PermanentFailureRecord.FailureCategory.BUSINESS_RULE_VIOLATION,
                PermanentFailureRecord.BusinessImpact.HIGH, "Cannot offboard - active accounts exist: " + exceptionMessage);
        }
        if (containsAny(lowerMessage, "outstanding balance", "non-zero balance")) {
            return RecoveryDecision.criticalReview("Offboarding blocked - outstanding balance requires review: " + exceptionMessage);
        }
        if (containsAny(lowerMessage, "duplicate", "already offboarded")) return RecoveryDecision.discard("Already offboarded");
    ...
    protected String maskPii(String payload) {
        try {
            ...
            if (root.isObject()) {
                ...
                maskField(node, "reason", "[REDACTED]");
