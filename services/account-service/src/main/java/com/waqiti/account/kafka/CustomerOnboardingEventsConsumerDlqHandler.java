...
@Service
@Slf4j
public class CustomerOnboardingEventsConsumerDlqHandler extends BaseDlqHandler<Object> {
    private static final String HANDLER_NAME = "CustomerOnboardingEventsConsumer";
    ...
    @KafkaListener(topics = "${kafka.topics.customer-onboarding-events.dlq:customer.onboarding.events.dlq}",
    ...
    protected RecoveryDecision classifyFailure(Object event, String exceptionMessage, String exceptionClass,
        ...
        if (containsAny(lowerMessage, "kyc", "compliance", "sanctioned", "pep")) return RecoveryDecision.forComplianceBlock(exceptionMessage, retryAttempt);
        if (containsAny(lowerMessage, "duplicate customer", "already onboarded")) return RecoveryDecision.discard("Duplicate onboarding");
        if (containsAny(lowerMessage, "validation", "invalid data")) return RecoveryDecision.forDataValidationError(exceptionMessage);
        if (containsAny(lowerMessage, "deserialization", "json")) return RecoveryDecision.forCorruptedData(exceptionMessage);
        return retryAttempt < 3 ? RecoveryDecision.retry(exceptionMessage) : RecoveryDecision.criticalReview("Onboarding failed - customer waiting: " + exceptionMessage);
    }
    ...
    protected String maskPii(String payload) {
        try {
            ...
            if (root.isObject()) {
                ...
                maskField(node, "address", "[REDACTED]");
