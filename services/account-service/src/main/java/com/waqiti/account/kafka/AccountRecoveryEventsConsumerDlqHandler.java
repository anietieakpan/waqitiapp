...
@Service
@Slf4j
public class AccountRecoveryEventsConsumerDlqHandler extends BaseDlqHandler<Object> {
    private static final String HANDLER_NAME = "AccountRecoveryEventsConsumer";
    ...
    @KafkaListener(topics = "${kafka.topics.account-recovery-events.dlq:account.recovery.events.dlq}",
    ...
    protected RecoveryDecision classifyFailure(Object event, String exceptionMessage, String exceptionClass,
        ...
        if (containsAny(lowerMessage, "security", "verification failed", "invalid token")) {
            return RecoveryDecision.criticalReview("Security verification failed - potential account takeover: " + exceptionMessage);
        }
        if (containsAny(lowerMessage, "duplicate", "already recovered")) return RecoveryDecision.discard("Already recovered");
    ...
    protected String maskPii(String payload) {
        try {
            ...
            if (root.isObject()) {
                ...
                maskField(node, "recoveryToken", "[REDACTED]");
                maskField(node, "securityAnswer", "[REDACTED]");
