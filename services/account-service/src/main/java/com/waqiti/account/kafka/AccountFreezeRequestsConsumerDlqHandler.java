...
@Service
@Slf4j
public class AccountFreezeRequestsConsumerDlqHandler extends BaseDlqHandler<Object> {
    private static final String HANDLER_NAME = "AccountFreezeRequestsConsumer";
    ...
    @KafkaListener(topics = "${kafka.topics.account-freeze-requests.dlq:account.freeze.requests.dlq}",
    ...
    protected RecoveryDecision classifyFailure(Object event, String exceptionMessage, String exceptionClass,
        ...
        if (containsAny(lowerMessage, "already frozen", "duplicate")) return RecoveryDecision.discard("Already frozen");
        if (containsAny(lowerMessage, "pending transaction")) {
            if (retryAttempt < 2) return RecoveryDecision.retry("Waiting for transaction settlement");
            return RecoveryDecision.highPriorityReview("Cannot freeze - pending transactions: " + exceptionMessage);
        }
