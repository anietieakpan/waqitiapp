...
@Service
@Slf4j
public class AccountStatusChangeEventsConsumerDlqHandler extends BaseDlqHandler<Object> {

    private static final String HANDLER_NAME = "AccountStatusChangeEventsConsumer";
    ...
    @KafkaListener(
        topics = "${kafka.topics.account-status-change-events.dlq:account.status.change.events.dlq}",
    ...
    protected RecoveryDecision classifyFailure(Object event, String exceptionMessage, String exceptionClass,
        ...
        if (containsAny(lowerMessage, "invalid transition", "invalid status", "cannot change")) {
            return RecoveryDecision.forInvalidState(exceptionMessage);
        }
        if (containsAny(lowerMessage, "duplicate", "already in status")) {
            return RecoveryDecision.discard("Duplicate status change");
        }
        if (containsAny(lowerMessage, "deserialization", "json")) {
            return RecoveryDecision.forCorruptedData(exceptionMessage);
        }
        return retryAttempt < 3 ? RecoveryDecision.retry(exceptionMessage)
            : RecoveryDecision.highPriorityReview("Max retries exceeded: " + exceptionMessage);
    }

    @Override
    protected boolean attemptRecovery(DlqRetryRecord retryRecord) {
        log.warn("[DLQ-{}] Recovery not yet implemented", HANDLER_NAME);
        return false;
    }
    ...
    protected String maskPii(String payload) {
        try {
            ...
            log.error("[DLQ-{}] PII masking failed", HANDLER_NAME, e);
        }
    ...
    private void maskField(ObjectNode node, String field, String mask) {
        if (node.has(field)) node.put(field, mask);
    }
