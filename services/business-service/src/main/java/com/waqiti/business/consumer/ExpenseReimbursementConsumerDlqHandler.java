...
/**
 * DLQ Handler for ExpenseReimbursementConsumer
 *
...
@Service
@Slf4j
public class ExpenseReimbursementConsumerDlqHandler extends BaseDlqConsumer<Object> {

    private final DlqRetryService dlqRetryService;

    public ExpenseReimbursementConsumerDlqHandler(
    ...
    @PostConstruct
    public void init() {
        initializeMetrics("ExpenseReimbursementConsumer");
    }

    @KafkaListener(
        topics = "${kafka.topics.ExpenseReimbursementConsumer.dlq:ExpenseReimbursementConsumer.dlq}",
    ...
    protected DlqProcessingResult handleDlqEvent(Object event, Map<String, Object> headers) {
        try {
            log.info("Processing DLQ event for ExpenseReimbursementConsumer: headers={}", headers);
            ...
            // CRITICAL: Expense reimbursements are financial transactions - must not be lost
            dlqRetryService.persistFailedMessage(
                    "ExpenseReimbursementConsumer",
                    topic,
                    partition,
                    offset,
                    messageKey,
    ...
    protected String getServiceName() {
        return "ExpenseReimbursementConsumer";
    }
}
